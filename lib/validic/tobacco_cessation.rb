...
module Validic
  module TobaccoCessation
    ...
    #
    # @return success
    def create_tobacco_cessation(user_id, activity_id, options={})
      options = {
        user_id: user_id,
    ...
    ##
    # Update TobaccoCessation base on `access_token` and `authentication_token`
    #
    # @params :access_token - *required if not specified on your initializer / organization access_token
    # @params :authentication_token - *required / authentication_token of a specific user
    #
    ...
    def update_tobacco_cessation(user_id, activity_id, options={})
      ...
      response = put_to_validic('tobacco_cessation', options)
      response if response
    end

    ##
    # Delete Tobacco Cessation record
    #
    # @params :access_token - *required if not specified on your initializer / organization access_token
    # @params :authentication_token - *required / authentication_token of a specific user
    #
    # @return success
    def delete_tobacco_cessation(user_id, activity_id, options={})
      options = {
        user_id: user_id,
        activity_id: activity_id,
        access_token: options[:access_token] || Validic.access_token,
      ...
      response = delete_to_validic('tobacco_cessation', options)
      response if response
    end
